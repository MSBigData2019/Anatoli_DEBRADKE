...
def last2(string):
    ...
    for i in range(0, len(string)-1):
        susbtring = string [i:i+2]
        if susbtring == pattern:
            count += 1
    return count
...
def occurences(text):
    ...
    word = text.lower()
    # Récupère la liste des caractères uniques
    char_list = set(word)
    # Compte le nombre d'occurence pour chaque caractères
    char_dic = {}
    for c in char_list:
        char_dic[c] = word.count(c)
...
def length_words(array):
    sizelist = list(map(lambda x: len(x), array))
    return sizelist
